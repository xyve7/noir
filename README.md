...
Additionally, building an ISO with `make all` requires `xorriso` (and `sgdisk` for x86-64 targets only), and building a HDD/USB image with `make all-hdd` requires `mtools` and `sgdisk`.

`sgdisk` is usually from `gdisk` or `gptfdisk` packages.
